...
async function submitForm(userInputs) {
    ...
    if (userInputs.requestComingFrom === 'searchBox') {
        ...
        url = userInputs.selectedOption === 'books' ?
            `https://openlibrary.org/search.json` :
            `https://api.themoviedb.org/3/search/${userInputs.selectedOption}`;
        options = {
            ...options,
            url: url,
            params: userInputs.selectedOption === 'books' ?
                {
                    title: query,
                    limit: 10,
                    language: 'eng'
    ...
    try {
        ...
        searchResults = userInputs.selectedOption === 'books' ?
...
export { categories, searchResults, validateForm, submitForm };