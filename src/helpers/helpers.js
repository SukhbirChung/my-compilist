...
async function checkIfUserLoggedIn() {
    ...
    try {
        ...
        loggedInUserInfo.username = response.data.username[0].toUpperCase() + response.data.username.slice(1);
        loggedInUserInfo.compilistList = response.data.compilistList;
        return response;
    }
    catch (err) {
        return err.response;
...
async function logout() {
    ...
    try {
        ...
        loggedInUserInfo.compilistList = '';
        return response;
    }
    catch (err) {
        return err.response;
...
async function submitForm(userInputs) {
    ...
    if (userInputs.requestComingFrom === 'searchBox') {
        ...
        } else if (selectedOption === 'documentaries') {
            url = `https://api.themoviedb.org/3/search/tv`;
        } else {
            url = `https://api.themoviedb.org/3/search/${selectedOption}`;
        }
...
async function addToCollection(category, id) {
    const options = {
        method: 'POST',
        url: 'http://localhost:3000/addToCollection',
        withCredentials: true,
        data: {category, id}
    }

    if (category === 'movie') {
        options.data.category = 'movies';
    } else if (category === 'tv') {
        options.data.category = 'shows';
    }
